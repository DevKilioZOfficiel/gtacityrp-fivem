...
$(document).ready(function () {
    ...
    if (event.data.action == "open") {
      ...
      for (const [key, value] of Object.entries(event.data.station)) {
        ...
        if (value.stationNumber == currentStation) {
          myElement =
            myElement +
            `<p class="priceBtn" style="margin-top: 5px;"><i class="fas fa-map-marker-alt"></i> Votre station</p>`;
        } else {
          myElement =
            myElement + `Le prix: <p class="priceBtn">$${value.price}</p>`;
        }
        myElement =
          myElement +
          `
      ...
      $("#currentStation").html(`Pas de station sélectionné`);
      $("#currentPrice").html(`$0`);
      ...
      intv = setInterval(() => {
        curTest = curTest - 1;
        $("#timer").html(`${curTest} secondes restantes...`);
      }, 1000);
  ...
  $(document).on("click", ".buyButton", function () {
    if (currentNumber != currentStation) {
      ...
      $(".stationButtons").empty();
      $.post(
        "https://vms_subway/action",
        JSON.stringify({ action: "transport", station: currentNumber })
      );
    }
  });
});
